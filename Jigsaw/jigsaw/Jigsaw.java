...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		Vector<JigsawNode> nodeNeedadd = new Vector<JigsawNode>();

		while(this.isCompleted == false && this.openList.isEmpty() == false)
		{
			JigsawNode theTop = this.openList.elementAt(0);

			if(theTop.equals(this.endJNode))
			{
				isCompleted = true;
				this.currentJNode = theTop;
			...
			pw.println("Searching...Number of searched nodes:" + this.closeList.size() + "Current state:" + theTop.toString());
			System.out.println("Searching...Number of searched nodes:" + this.closeList.size() + "Current state:" + theTop.toString());

			this.openList.removeElementAt(0);
		
			this.closeList.addElement(theTop);

			nodeNeedadd = findFollowJNodes(theTop);

			while(nodeNeedadd.isEmpty() == false)
			{
				this.openList.addElement(nodeNeedadd.elementAt(0));
				nodeNeedadd.removeElementAt(0);
			}
		}
	...
	private void estimateValue(JigsawNode jNode)
	{
		...
		JigsawNode processNode = jNode;
		dimension = processNode.getDimension();
		int[] nodesState = processNode.getNodesState();
		
		for(int rowNow = 1, gridNow = 1; rowNow <= dimension; rowNow++)
		{
			
			for(int colNow = 1; colNow <= dimension; colNow++, gridNow++)
			{
				int valueofGrid = nodesState[gridNow];
				int rowinTrue = valueofGrid/dimension + 1;
				if(valueofGrid%dimension == 0) rowinTrue--;
				int colinTrue = valueofGrid%dimension;
				if(colinTrue == 0) colinTrue = dimension;
				
				rowColDif += Math.abs(rowNow - rowinTrue) + Math.abs(colNow - colinTrue);
				
				if(colNow != valueofGrid) firstRowDif++;
				
				if(colNow < dimension &&valueofGrid+1 != nodesState[gridNow + 1])
					rowNextDif++;
				if(rowNow < dimension && valueofGrid + dimension != nodesState[gridNow + dimension])
					rowNextDif++;
			}
		}
		
		result = 7 * rowColDif + 5 * rowNextDif +2* firstRowDif;
